...
**2026-07-08 — 🪟 INSTALACIÓN LIMPIA · P1 "TRACK WINDOWS" ✅ (programa `BACKLOG §3`, objetivo (2))** (`feat/install-windows`). Lleva la instalación offline a **Windows (Docker Desktop / WSL2 con contenedores Linux)** reflejando el `install.sh` YA afinado. **El programa "instalación limpia" queda CERRADO** para la matriz soportada (Linux x86-64 + Windows). **Enfoque (decisión del dueño):** `install.ps1` **NATIVO** completo (espejo del install.sh), no un hand-off a WSL2 — el agente recomendó el hand-off (cero duplicación) y **registró su objeción de duplicación** en DECISIONS; mitigada haciendo el `install.ps1` un espejo fiel (misma estructura/mensajes/openssl/lógica IP↔default_sni). **(1) Huella L1 en Windows — validado EN VIVO:** `/etc/machine-id` **NO es fiable** bajo Docker Desktop (el distro `docker-desktop` no lo tiene; el valor de la VM interna es opaco y se **regenera al actualizar DD / resetear WSL** ⇒ rompería el node-lock en un update rutinario). Se **ancla al `MachineGuid`** del host Windows (`HKLM\…\Cryptography`), escrito en `license\machine-id` (re-derivado cada corrida) + `LICENSE_MACHINE_ID_FILE=/app/license/machine-id` en el `.env`; el colector (Linux dentro del contenedor) lo lee por esa ruta. **CERO cambios al compose** (default Linux `/etc/machine-id` intacto ⇒ smoke Linux no se toca). Node-lock preservado (una copia reejecutada en otra máquina re-deriva su GUID). **(2) Permisos:** el `chown uid 1000` de Linux no aplica al FS Windows; `install.ps1` corre un **probe `docker run -u 1000:1000`** que valida escritura en `./license` **antes** de arrancar (no debilita CIS 5.12). **(3) `install.ps1`/`doctor.ps1` nativos** (ASCII-only para consola de terreno; `.env`/Caddyfile en LF para evitar el trap CRLF de `--env-file`): preflight (daemon linux/amd64 — aborta si modo Windows-containers; openssl requerido, simétrico con Linux), cosign best-effort, `SHA256SUMS` vía `Get-FileHash`, `docker load`, `.env`+secretos openssl, modo a/b con Caddyfile+cert self-signed autogenerados (default_sni solo si IP), up + healthcheck, y `-Check` (doctor PASA/FALLA). Empacados en `make-bundle` (sellados en `SHA256SUMS`). **(4) Smoke:** `scripts/smoke-install-windows.ps1` (manual/self-hosted; CI `windows-latest` no corre contenedores Linux de forma fiable). **MODO A validado EN VIVO** sobre Docker Desktop real (Win 11, docker 29.5.3, linux/amd64): 1ª pasada exit 2 + machine-id/`LICENSE_MACHINE_ID_FILE` OK, 2ª pasada integridad 37 archivos + probe uid1000 OK + up + **API healthy** + `/api/health` 200 + **doctor 14× PASA**. Modos b-host/b-ip: no ejercidos en vivo aquí (puerto 80 ocupado en la máquina) — su lógica mode-b es idéntica al install.sh (cubierta por el smoke Linux) y el script los corre en un host limpio. **El plumbing huella-por-archivo se cubre TAMBIÉN en el smoke Linux** (escenario 4 nuevo: `LICENSE_MACHINE_ID_FILE`→archivo montado ⇒ API healthy). **Cero código app** (el colector ya soportaba `LICENSE_MACHINE_ID_FILE`) ⇒ suite TS intacta. Docs: DECISIONS (a–e Track Windows), SUPPORTED_PLATFORMS §Windows (de "en preparación" a soportado-con-salvedades), INSTALL_OFFLINE §12, PRUEBA_PILOTO §Windows, memoria [[install-packaging-robustness]]. **PUBLICADO: tag v0.1.21** (release VERDE run 28989575352: build+firma cosign → gate Trivy → bundle → deploy EC2; `lyra-watchlog-v0.1.21.tar.gz` 1.07 GB **con install.ps1** adjunto al GitHub Release; PRUEBA_PILOTO actualizada a v0.1.21). **Pendiente menor (no bloqueante):** correr el smoke Windows completo (a+b-host+b-ip) en un host con 80/443/8080 libres — lo cubre la prueba manual del dueño en Windows.

**2026-07-08 — 🧰 INSTALACIÓN LIMPIA · P0 "install.sh AUTOREPARABLE + doctor" ✅ (programa `BACKLOG §3`) ⇒ en `main` (smoke VERDE run 28984487292)** (`feat/install-autoreparable-doctor`). Cierra el objetivo (1) del plan del dueño 2026-07-08: eliminar los pasos manuales frágiles que causaron la mitad de los tropiezos del 1er piloto (crear el cert con `openssl` + editar `edge/Caddyfile.edge` por `sed`). **Ahora el operador SOLO fija `EDGE_MODE`+`APP_PUBLIC_URL` en `.env`; el instalador hace el resto.** **(1) Borde AUTOGENERADO (modo b):** `install.sh` **regenera `edge/Caddyfile.edge`** desde `APP_PUBLIC_URL` en cada corrida (cabecera "GENERADO, no editar"; OK con `SHA256SUMS` porque `edge/` ya está excluido), poniendo `default_sni` **SOLO cuando el host es IP** (`is_ipv4`; por hostname el SNI viaja y el matching estricto es lo deseable). El Caddyfile del repo quedó como REFERENCIA. **(2) Certificado AUTOGENERADO:** cert self-signed (825 d, SAN `IP:`/`DNS:` según host) si faltan `certs/`; **el corporativo tiene prioridad** si lo colocas. Preflight del cert: **cert↔llave = die** (compara clave pública, robusto RSA/EC); **SAN cubre host = AVISO** (wildcard/CA legítimos no lo contienen literal); vencido = aviso. **(3) Preflight DURO:** arquitectura del daemon ≠ amd64 = die (matriz), puertos del modo ocupados por OTRO proceso = die (idempotente: si el propio stack ya escucha, se omite), `.env` incompleto = die. **(4) `doctor` = `install.sh --check`** (+ wrapper `doctor.sh`, empacado por make-bundle): reporte PASA/FALLA no destructivo (arch, docker/compose, puertos, cert, coherencia `default_sni`, salud por contenedor, alcance HTTP/S); sale 1 si hay FALLA. **(5) Smoke extendido a la ruta AUTO** (sin openssl/sed manual): modo a + **modo b hostname** (SNI 200 + **prueba NEGATIVA por IP**: sin default_sni ⇒ rechazo) + **modo b IP** (default_sni ⇒ 200) + `doctor` VERDE en los 3. **GOTCHA que atrapó el smoke:** tras el `chown` a uid 1000, `key.pem` queda 600/uid1000 ⇒ un instalador/doctor **no-root NO la lee** ⇒ `cert_key_match` daba falso "NO corresponden" (falso FALLA en doctor + falso die en la 3ª corrida idempotente). Fix: `key_readable()` gatea la verificación; si no es legible, se omite con nota (ya validada al crearla). **`docs/SUPPORTED_PLATFORMS.md` CREADO** (Linux x86-64 soportado+probado; Windows "en preparación"; ARM/macOS/k8s/IPv6 fuera). **Cero código app** ⇒ suite TS intacta (330/330, sin cambios; solo shell/CI/docs). Docs: DECISIONS (a–e), PRUEBA_PILOTO (menos pasos), INSTALL_OFFLINE, BACKLOG §3, memoria [[install-packaging-robustness]]. **SIGUIENTE SESIÓN = objetivo (2): track Windows** (`install.ps1`/WSL2 + `/etc/machine-id` huella L1 + permisos FS Windows + smoke propio + sección Windows de SUPPORTED_PLATFORMS).

**2026-07-08 (cierre) — 🔏 CEREMONIA cosign de PROD + PRIMER RELEASE FIRMADO ✅ ⇒ tag v0.1.20 PUSHEADO** (cierra la deuda de E3). Completa la única deuda que dejó E3: la **ceremonia de custodia del par cosign de PRODUCCIÓN** (modelo L3, igual que la clave de emisión de licencias). **(1) Keygen** con **cosign v2.4.3** (pin; NO v3) vía imagen Docker `gcr.io/projectsigstore/cosign:v2.4.3` en la máquina de confianza del dueño → par en `~/.lyra-license/cosign/` (FUERA del repo), **privada cifrada** con passphrase de alta entropía (`openssl rand -base64 32`) que el dueño custodia en su gestor. **Self-test offline** (sign-blob + verify-blob) = "Verified OK" ANTES de tocar nada. **(2) Solo la PÚBLICA** committeada (`scripts/license/cosign/cosign.pub`); verificado que `cosign.key` **JAMÁS** entró al árbol (`.gitignore` línea 59 la bloquea; vive fuera del repo). **(3) GitHub Secrets** `COSIGN_PRIVATE_KEY` (contenido de `cosign.key`, por stdin) + `COSIGN_PASSWORD` cargados en `victorrubilarc/lyra-platform` vía `gh` (portable, autenticado con el token del credential manager). **(4) Tag v0.1.20 PUSHEADO** con OK explícito del dueño → `release.yml` **completed/success** los 6 jobs: build+**firma por digest** de las 3 imágenes (paso "Firmar la imagen por digest" ✓, `HAS_COSIGN=true`) · **gate Trivy** ✓ · **bundle** firmado (`sign-blob` del SHA256SUMS) + **SBOM** ✓ · **deploy EC2** ✓ (el demo corre v0.1.20). **(5) PRUEBA DE FUEGO — verificación REAL del primer release firmado:** `cosign verify --key cosign.pub --insecure-ignore-tlog=true` sobre las 3 imágenes GHCR por digest = **3/3 firmas válidas**; descarga del `.tar.gz` del Release (1.09 GB) → `verify-blob` del `SHA256SUMS` con `--network none` = **"Verified OK"** (100% offline) + `cosign.pub` del bundle **idéntica** a la del repo; **prueba negativa** (SHA256SUMS alterado) → **rechazado** (`invalid signature`); **7/7 SBOM CycloneDX 1.7** adjuntos al Release bien formados. **Gotchas de la sesión:** GHCR privado ⇒ `cosign verify` con `--registry-username/--registry-password` (no `--registry-token`, que pasa el PAT como Bearer y GHCR lo rechaza) · el marcador de éxito de `verify` (imagen) es "signatures were verified against the specified public key", NO "Verified OK" (ese es de `verify-blob`) · Docker/Windows: montar con `MSYS_NO_PATHCONV=1` + ruta `C:/...`. **Cero código app tocado** ⇒ suite intacta. **PENDIENTE EC2 de H2 sigue AGENDADO** (no se tocó). **Siguiente: H3 enterprise (SIEM + hook AV + SSO/AD + EDR host) o cierre del programa (pentest de tercero + security.txt + paquete de evidencia).**

**2026-07-08 — 🔗 E3 · "CADENA DE SUMINISTRO VERIFICABLE" ✅ (backups cifrados + SBOM + firma cosign) ⇒ tag v0.1.20 (pendiente OK del dueño para pushear)** (`feat/e3-cadena-suministro`). Cierra el objetivo de `SECURITY §9.5`: que un cliente/auditor compruebe **sin confiar a ciegas** que el software (a) vino de ITESICWS y no fue alterado (firma), (b) qué contiene (SBOM), (c) que sus respaldos son ilegibles si roban el disco (cifrado). **Nada rompe el air-gap, el sello L5 ni la máquina de estados de licencia; ninguna clave privada entra al repo/imagen/.env.** Decisiones (a)–(h) aprobadas antes de codificar (DECISIONS 2026-07-08 E3). Orden ejecutado: backups + SBOM (bajo riesgo), cosign al final. **(1) BACKUPS CIFRADOS (age asimétrico):** `onprem/backup.sh` cifra el `pg_dump -Fc` en el PIPE (el claro nunca toca disco) con la clave PÚBLICA del cliente cuando se define `BACKUP_AGE_RECIPIENT`; descifrar exige la identidad PRIVADA que el cliente **custodia FUERA del host** (disco robado = inútil; host comprometido no lee sus propios respaldos). `onprem/restore.sh` NUEVO (autodetecta stack demo/prod↔standalone; `--test` restaura a BD descartable sin tocar lo vivo, `--live` con confirmación). `age` estático (v1.3.1, pin sha256) viaja en el bundle (`tools/age`) — air-gap sin dependencia de red. **Sin destinatario ⇒ dump en claro + aviso** (retrocompat demo/dev/EC2). **Probado el ciclo COMPLETO** cifrar→descifrar→restaurar: **42/42 filas** íntegras (`cliente_42` verificado); identidad equivocada FALLA; el `.age` es `age-encryption.org/v1`, no un PGDMP. **(2) SBOM CycloneDX:** `scan-images.sh` emite un CycloneDX JSON por imagen (7) reusando Trivy (`--format cyclonedx`); viaja en el paquete (`SECURITY/sbom/`) **y** adjunto suelto al GitHub Release (`*-sbom.tar.gz`). Validado CycloneDX 1.7 bien formado. **(3) FIRMA cosign (key-based, offline):** `release.yml` firma cada imagen GHCR **por digest** (`cosign sign --tlog-upload=false`, artefacto OCI) + `make-bundle` firma el **`SHA256SUMS`** del paquete con `sign-blob` (autentica el bundle ENTERO sin registro — resuelve el gotcha `save/load`). `install.sh` verifica la firma (best-effort: si el host no trae cosign, el auditor la verifica en su estación) ANTES del `docker load`; `update.sh` verifica las imágenes de app **por digest** tras el pull (best-effort, aborta si el toolchain está y falla). **cosign v2.4.3** (pin; v3 rompió `--tlog-upload=false`). Custodia modelo L3: privada en `~/.lyra-license/cosign/` + GitHub Secrets `COSIGN_PRIVATE_KEY`/`COSIGN_PASSWORD`; pública committeada (`scripts/license/cosign/cosign.pub`, README de ceremonia). **Sin el secret la firma se OMITE sin romper** (deuda hasta la keygen del dueño, como L1 pre-L3). Flujo validado en vivo con llaves desechables: "Verified OK" auténtico, **rechazo del SHA256SUMS re-firmado por atacante sin la privada**. **(4) Registro privado auth-ready:** `update.sh` hace login best-effort a GHCR privado con `WL_REGISTRY_USER`/`WL_REGISTRY_TOKEN` (sin ellos = flujo público intacto); modelo = GHCR privado + PAT fine-grained read-only revocable por socio (NO registro self-hosted: la planta air-gapped no usa registro). **El flip de visibilidad de GHCR es acción GATED del dueño** (rompería el auto-deploy del EC2). **Verde:** typecheck/lint/build/test **API 330/330 · web 26/26** (cero código app tocado — solo shell/CI/gitignore/docs). Pruebas REALES: ciclo de backup cifrado 42/42, SBOM CycloneDX 1.7, cadena firma+integridad del bundle con 2 casos de manipulación. Docs: DECISIONS (a–h), SECURITY §9.1/9.2/9.3/9.8, DEPLOYMENT (backup/restore cifrado + verificación de firma + registro/tokens), INSTALL_OFFLINE (§1/§10/§11), `scripts/license/cosign/README.md` NUEVO, `.gitignore` endurecido. **PENDIENTE EC2 de H2 sigue AGENDADO** (no se tocó el EC2). **Tag v0.1.20 NO pusheado** (auto-despliega EC2 ⇒ consultar al dueño; decisión h). **Siguiente: H3 enterprise (SIEM + hook AV + SSO/AD + EDR host) o cierre del programa (pentest de tercero + security.txt + paquete de evidencia).**

**2026-07-07 — 🛡️ HARDENING H2 · "CIS DOCKER + GATE DE SUMINISTRO" ✅ (Tanda B / E4 del sub-épico a prueba de balas) ⇒ tag v0.1.19 (LOCAL, sin pushear hasta OK del dueño)** (`feat/h2-cis-hardening`). Endurece el runtime de contenedores a estándar **CIS Docker Benchmark** y convierte Trivy en **gate real**, sin romper la ceremonia air-gapped ni el sello L5. Decisiones (a)–(h) aprobadas antes de codificar (DECISIONS 2026-07-07 H2). **(1) Contenedores NON-ROOT:** `USER node` (uid 1000) en `Dockerfile.api` stages runtime **y** migrate; `USER app` (uid 1000) en `Dockerfile.web`. La ceremonia de licencia funciona non-root: la API (uid 1000, read_only) **escribió `solicitud.lreq`+`setup-token`** en el smoke — el bind `./license` se cede a 1000 en `install.sh`/`update.sh`. `migrate` (non-root + read_only + cap_drop ALL) corrió `migrate deploy`+`tsx seed` (esbuild, TMPDIR=/tmp sobre tmpfs) exit 0. **(2) Compose endurecido** (standalone + demo/prod, Opción A): `no-new-privileges` en todos, `cap_drop:[ALL]` (app+infra), `read_only:true` (api/migrate/web/edge/redis/minio) con `tmpfs` para lo escribible, `cap_add:[NET_BIND_SERVICE]` + sysctl `ip_unprivileged_port_start=0` en web/edge (**gotcha:** el binario Caddy lleva la file-cap ⇒ con bounding vacío el execve daba EPERM). **(3) Pins de infra:** demo/prod por `@digest`; standalone por TAG (**gotcha:** `docker save/load` no preserva RepoDigests ⇒ el pin real es el tar del bundle; `make-bundle` pullea por digest→retag→save); MinIO sale de `latest` al RELEASE inmutable. **(4) Trivy GATE en `release.yml`** (job `scan`, CRÍTICO-solo, bloquea bundle+deploy) + **`.trivyignore`** con 7 CVE justificados (todos base/Go-de-tercero/MinIO-interno, 0 de app). **(5) Doc iptables/firewall** para el equipo de redes (DEPLOYMENT §host). **Honestidad:** pinear MinIO NO baja sus CVE (ya es la última release sin fix); el win es non-root/read_only/cap_drop + gate + clasificación. Delta Trivy vs E2: MinIO 6→4 CRÍT, resto sin cambio (base sin parche upstream). **Verificado:** smoke en vivo mode-a :3412 + mode-b :8443 (api healthy, migrate exit 0, web/edge HTTP 200, HTTPS por el borde 443 non-root) + `docker inspect` (User/RO/cap/nnp) + gate Trivy VERDE + typecheck/lint/build/test **330/330**. **Sigue: E3 (cosign firma+verificación + pull por digest de app + SBOM CycloneDX + backups cifrados + registry privado).**
