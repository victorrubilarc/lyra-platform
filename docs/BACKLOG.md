...
| **Fase 2.8 Alcance por PLANTILLA (2.º eje ABAC)** (`TemplateScope` + `getAccessibleTemplateIds`/`assertTemplateInScope` + filtro picker/grilla + `PUT users\|roles/:id/template-scope` + options + `TemplateScopePicker`) | `feat/alcance-plantilla` → `main` | ⬜ por fusionar+publicar en esta sesión | merge a `main` + push |
