...
| **Catálogo de objetos premium · Ola 3** (adjuntos / terreno, MinIO) | ✅ | `feat/objetos-ola3`: **evidencia** con object storage on-prem. `StorageService` abstracto + `MinioStorageService`. Un `FieldType ATTACHMENT` + presets → `dataType FILE_ARRAY` (valor `descriptor[]`): **foto/cámara · archivo · nota de voz · croquis** + **escáner QR** (`config.scan` sobre TEXT, NO archivo). Subida **PROXIED** (`@fastify/multipart`, choke-point de validación) a `entries/{id}/{fieldKey}/…`; descriptor en `LogEntryValue.value` (NUNCA URL); descarga = **presigned GET** con la ABAC de `getDetail`; pertenencia por prefijo + delete-on-remove + VOID limpia huérfanos; entrada sellada inmutable. Migración ALTER enum. Contracts 222 · API 234 · smoke 26/26. **Olas 4–5 pendientes** (tabla/grupo repetible · origen de datos). Falta smoke visual. **Diferido:** antivirus · WORM · thumbnails · sweeper · presigned directo. |
...
- **Smokes VISUALES del dueño** (BACKLOG §4): grilla 2.8.1, diagrama de flujo, SLA/atrasos, **motor de reglas**, **VOID + ruta de edición (2.8.2)**, **catálogo de objetos · Olas 1, 2 y 3** (Ola 3 = adjuntos/evidencia + escáner QR).
