...
|---|---|---|---|---|---|
| 1 | bug | Workspace / sesión | Media | Al cambiar de usuario en el mismo navegador, las **pestañas abiertas del usuario anterior persisten**. El estado de pestañas es propio de cada usuario: al iniciar sesión debería entrar **limpio** (o restaurar solo las del usuario que entra). Posible fuga de contexto entre cuentas. | Abierto |
| 2 | mejora UX | Auth / cambio de contraseña | Baja | En la pantalla de **cambio de contraseña** faltan los toggles "**mostrar/ocultar**" (ícono de ojo) en los inputs de contraseña. | Abierto |
| 3 | bug | Estructura | Media | El panel del nodo muestra **o hijos o equipos** (excluyente por `isLastLevel`, `NodeDetail.tsx:352`). Un equipo colgado de un nodo **intermedio** (no último nivel) queda **invisible** en la UI: p. ej. el Molino SAG en «Molienda SAG» no se ve porque ese nodo tiene Líneas debajo. Debería mostrar **ambos** (hijos + equipos) o pestañas. Detectado al añadir el 4º nivel «Línea». | Abierto |
| 4 | bug (i18n) | Seguridad / matriz de permisos | Baja | Los encabezados de grupo de la matriz mezclan idiomas: solo `security/structure/users/roles` están traducidos (`es-CL.ts permGroups`), el resto (templates, workflows, incidents, notifications, opscalendar, referencedata, settings, schedules, logbook, opsperiod) cae al string crudo en **inglés** (`PermissionMatrix.tsx:34`). Faltan claves en `permGroups`. | Abierto |
| 5 | mejora UX / seguridad | Auth / propagación de permisos | Media | **El backend opera bien** (devuelve 403 correcto; el caché Redis `authz:perms:<userId>` reflejó fielmente el set real — sin bug de caché). El problema es el **cliente**: el gate usa la foto de `session.permissions` del login (`use-permissions.ts`); al cambiar roles/permisos de un usuario con **sesión activa**, su menú/pantallas **siguen visibles** hasta refrescar/re-login y puede **abrir** una pantalla ya no permitida. Mitigación: invalidar sesión o refetch del checker ante cambios sensibles (o forzar re-login en revocaciones). | Abierto |
| 6 | bug | Manejo de errores (web) | Media | Ante **403 del backend**, la UI **no muestra ningún aviso**: la pantalla queda "cargando"/vacía con KPIs en **0/0/0** mientras la consola se llena de `403 (Forbidden)` (repro: entrar a Bitácoras sin `logentry:view`). Falta estado de error / empty-state "Sin acceso" / toast. Transversal a las grillas que dependen de `useQuery`. | Abierto |
